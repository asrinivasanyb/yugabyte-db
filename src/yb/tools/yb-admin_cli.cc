...
Status create_change_data_stream_action(
  ...
  std::string consistent_snapshot_option = yb::ToString("NOEXPORT_SNAPSHOT");
  ...
  if (args.size() > 3) {
    ...
    if (uppercase_consistent_snapshot_option != yb::ToString("USE_SNAPSHOT") &&
        uppercase_record_type != yb::ToString("NOEXPORT_SNAPSHOT")) {
      return ClusterAdminCli::kInvalidArguments;
    }
